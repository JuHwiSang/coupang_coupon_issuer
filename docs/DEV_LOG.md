...
**문제**: 쿠폰 타입 명칭이 일관성 없이 사용됨
- "다운로드쿠폰"은 "쿠폰" 접미사 포함
- "즉시할인"은 "쿠폰" 접미사 없음

**해결**: 모든 코드와 문서에서 "즉시할인쿠폰"으로 통일

**변경된 파일**:
1. **소스 코드**:
   - `reader.py`: 쿠폰 타입 파싱 로직, 검증 메시지
   - `issuer.py`: 쿠폰 타입 체크 조건문 (L218, L236)

2. **테스트 코드**:
   - `test_reader.py`: 테스트 데이터 및 검증 (~20개 테스트)
   - `test_issuer.py`: 테스트 데이터 및 검증 (~32개 테스트)
   - `test_cli.py`: CLI 테스트 데이터
   - `test_utils.py`: 한글 문자열 너비 계산 예시

3. **예제 스크립트**:
   - `generate_example.py`: 드롭다운 검증, 주석, 예제 데이터

4. **문서**:
   - `CLAUDE.md`: Excel 구조 설명
   - `USER_GUIDE.md`: 쿠폰 타입 설명, 예시

**하위 호환성**:
- `reader.py`에 정규화 로직 추가
- 기존 Excel 파일의 "즉시할인" 입력 → "즉시할인쿠폰"으로 자동 변환
- `test_reader.py`에 하위 호환성 테스트 추가 (`test_backward_compatibility_instant_coupon`)
...
1. **시작일 계산 로직 변경** (`issuer.py:206-224`):
   - 즉시할인: 오늘 0시 (기존 유지)
   - 다운로드쿠폰: 현재시각 + 1시간 10분 (신규)
   - 근거: +10초로는 API 에러 발생, 충분한 여유 확보

2. **종료일 계산 로직 변경** (`issuer.py:206-224`):
   - 즉시할인: 오늘 0시 + N일 (기존 유지)
   - 다운로드쿠폰: 오늘 자정 + N일 (신규)
   - 근거: 원래 스펙 복원 (기존 구현 오류 수정)

3. **최소구매금액 기본값 변경** (`reader.py:144`):
   - 기존: 1원
   - 변경: 10원
...
### 비개발자용 사용설명서 작성 (2025-12-26)

**목적**: 프로그래밍 지식이 없는 사용자도 쉽게 프로그램을 사용할 수 있도록 간단한 가이드 제공

**생성 파일**:
- `docs/USER_GUIDE.md` - 비개발자용 사용설명서

**주요 내용**:
1. **엑셀 파일 작성**: examples 폴더에서 복사, 형식 절대 변경 금지 강조
2. **서버 업로드**: 프로그램과 엑셀 파일을 리눅스 서버에 업로드 (방법은 사용자 선택)
3. **설치 과정**: 
   - `setup` 명령어 (시스템 준비, sudo 필요, 최초 1회)
   - `install` 명령어 (서비스 설치, sudo 불필요)
4. **관리 방법**: verify, 로그 확인, 수동 실행
5. **제거 방법**: uninstall, 완전 삭제

**주의사항 강조**:
- ⚠️ 프로그램 폴더 이동 금지 (install 후)
- 💰 쿠폰 예산 차감 (자유계약 예산에서 차감)
- 📊 엑셀 형식 엄수 (조금이라도 바꾸면 동작 안함)
- 📁 엑셀 파일명 고정 (`coupons.xlsx`)
- ✅ verify 명령어 활용 권장
- 🕐 타임존 (KST, UTC+9)
...
- 로그 파일 확인 방법
- 자주 발생하는 오류 및 해결 방법
- 개발자에게 문의 시 전달할 정보 (issuer.log, 프로젝트 폴더)
...
- ADR 017: 쿠폰 타입별 검증
- ADR 018: 한글 할인방식
- ADR 019: setup/install 분리
- ADR 020: 폴링 처리
- ADR 021: 9컬럼 구조
- ADR 022: 타임존 및 시작일 처리

**특징**:
- 명령어 최소화 (setup, install, verify, uninstall만 설명)
- 단계별 가이드 (1단계: 엑셀 작성 → 2단계: 업로드 → 3단계: 설치 → 4단계: 관리)
- 이모지 사용으로 가독성 향상
- 중요 주의사항 강조 표시
- 실제 사용 예시 포함
