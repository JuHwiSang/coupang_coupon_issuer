...
| **F** | **최소구매금액** | **쿠폰 사용 최소 구매 조건** | **"10000"** | **다운로드쿠폰 전용, 기본값 1** |
...
policy = {
    "minimumPrice": min_purchase_price,  # Excel 값 (기본값 1)
...
- **최소구매금액**: 기본값 1원 (제한 없음과 동일)
  - 사용자가 비워두면 모든 금액에서 사용 가능
  - 필요 시 명시적으로 입력
- **최대할인금액**: 필수 입력
  - 정률할인에서 중요한 값
  - 비즈니스 로직상 반드시 설정 필요
...
### 대안 2: 최소구매금액을 0원 기본값으로 (거부됨)

기본값을 0원으로 설정

- **장점**: "제한 없음"의 의미가 더 명확
- **단점**: API에서 1원 이상을 요구할 수 있음
- **거부 이유**: 1원이 더 안전하고, 실질적으로 동일한 효과
